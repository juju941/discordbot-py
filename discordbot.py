...
async def on_message(message):
    if message.author == client.user:
        ...
        if message.content == "f'{PREFIX}명령어":
        embed = discord.Embed(title="f'{PREFIX}명령어", description="아래의 명령어를 입력해보세요!", color=0xff0000)
        embed.add_field(name="f'{PREFIX}행운", value="행운을 올려보세요", inline=False)
        embed.add_field(name="f'{PREFIX}가이드북", value="가이드북 주소를 알려드려요!", inline=False)
        embed.add_field(name="f'{PREFIX}패치파일", value="패치파일 주소를 알려드려요!", inline=False)
        embed.add_field(name="f'{PREFIX}천사날개 조합식", value="천사날개 조합식을 알려드려요!", inline=False)
        embed.add_field(name="f'{PREFIX}알파윙 조합식", value="알파 윙 조합식을 알려드려요!", inline=False)
        embed.add_field(name="f'{PREFIX}진알파 조합식", value="眞-알파 윙 조합식을 알려드려요!", inline=False)
        embed.add_field(name="f'{PREFIX}케루빔 조합식", value="케루빔의 수호 날개 조합식을 알려드려요!", inline=False)
